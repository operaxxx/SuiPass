...
function validateEnvironment(env) {
  ...
  if (env.VITE_SUI_NETWORK && !validNetworks.includes(env.VITE_SUI_NETWORK)) {
    errors.push(
      `Invalid network: ${env.VITE_SUI_NETWORK}. Must be one of: ${validNetworks.join(", ")}`,
    );
  }
...
function showEnvironment(envName) {
  try {
    ...
    colorPrint(
      "red",
      `❌ Error reading ${envName} configuration: ${error.message}`,
    );
    process.exit(1);
  }
}
...
function createEnvironment(envName) {
  ...
  if (fs.existsSync(targetPath)) {
    colorPrint(
      "red",
      `❌ Environment file already exists: ${ENV_FILES[envName]}`,
    );
    process.exit(1);
  }
